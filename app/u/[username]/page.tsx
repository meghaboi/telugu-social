  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-6">
      ...
      <section className="rounded-2xl border border-white/10 bg-surface p-5">
        ...
        <div className="mt-2 flex gap-2"><Badge>{profile.city}</Badge></div>
      ...
      <section className="mt-6 space-y-3">
        ...
        {posts.map((post) => (
          <article key={post.id} className="rounded-xl border border-white/10 bg-surface p-4">
            <p className="mb-2 text-xs text-muted">{post.forums?.name} • {new Date(post.created_at).toLocaleString()}</p>
            <p>{post.content}</p>
          </article>
        ))}
      </section>
